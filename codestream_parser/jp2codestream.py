...
class JP2Codestream:
    ...
    def parse_data(self, buffer = None):
        ...
        while 1:	    
            count = 0
            while len(self.buffer) - self.pos >= 2 and \
                  (ord(self.buffer[self.pos + 0]) != 0xff or \
                   ord(self.buffer[self.pos + 1]) < 0x90):
                self.pos = self.pos + 1
                count = count + 1
    ...
    def read_unknown_marker(self):
        if self.marker == 0x00 or self.marker == 0x01 or self.marker == 0xfe or \
           self.marker >= 0xc0 and self.marker <= 0xdf:
            type = "ISO/IEC 10918-1"
        elif self.marker >= 0xf0 and self.marker <= 0xf6:
            type = "ISO/IEC 10918-3"
        elif self.marker >= 0xf7 and self.marker <= 0xf8:
            type = "ISO/IEC 14495-1"
        elif self.marker >= 0x30 and self.marker <= 0x3f:
            type = "segment-less"
        elif self.marker >= 0x50 and self.marker <= 0xff:
