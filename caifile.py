...
def parse_jumd_box(box):
    ...
    cai_box = {"box_type": "jumd"}

    box_type = buffer[0:16]
    box.print_indent("TYPE: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x" % \
           (box_type[ 0], box_type[ 1], box_type[ 2], box_type[ 3],
            box_type[ 4], box_type[ 5], box_type[ 6], box_type[ 7],
            box_type[ 8], box_type[ 9], box_type[10], box_type[11],
            box_type[12], box_type[13], box_type[14], box_type[15]))
    cai_box["type"] = buffer[0:16].hex()

    toggles= ord(buffer[16:17])
    box.print_indent("TOGGLES: %s" % bin(toggles))
    cai_box["toggles"] = toggles
    opt_start = 17
    ...
    if testBit(toggles,3):
        ...
        cai_box["signature"] = sig
    else:
        box.print_indent("No Signature")
        cai_box["signature"] = ""

    Cai_boxes.append(cai_box)
...
def parse_json_box(box,buffer):
    ...
    cai_box["data"] = s.decode('utf-8')
    Cai_boxes.append(cai_box)

def parse_superbox(box,boxtype):
    print(boxtype)
    box = JP2Box(box,box.infile)
    box.parse(cai_superbox_hook)
    
def cai_superbox_hook(box,id,length):
    global Cai_boxes

    print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
    print("cai_superbox_hook is called")
    print("id: " + id)
    print("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
